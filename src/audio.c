...
void audio_init_splash(AppState *as)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        float noise = (SDL_randf() * 2.f - 1.f) * 0.6f;
...
bool audio_init(AppState *as)
{
    ...
    as->splash_stream = SDL_CreateAudioStream(&spec, &spec);
    if (as->splash_stream) SDL_BindAudioStream(as->audio_dev, as->splash_stream);
    generate_quack(as);
...
{
    if (!as->splash_pcm || !as->audio_dev || !as->splash_stream) return;
    SDL_PutAudioStreamData(as->splash_stream, as->splash_pcm, as->splash_pcm_bytes);
}
...
void audio_cleanup(AppState *as)
{
    ...
    if (as->splash_stream) {
        SDL_DestroyAudioStream(as->splash_stream);
        as->splash_stream = NULL;
    }
    if (as->audio_dev) {
